...
void update_ui() {
    ...
    if(getLocalTime(&timeinfo)){
        ...
        // Log solo al primo aggiornamento riuscito
        if(first_update) {
            Serial.println("[UI] Primo aggiornamento orologio riuscito!");
            first_update = false;
        }
    } else {
        // DEBUG: Mostra che NTP non è ancora pronto
        if(first_update) {
            lv_label_set_text(lbl_time, "Sync...");
        }
    }
