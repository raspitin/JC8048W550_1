...
// Variabile esterna per il termostato (per comandarlo)
#include "thermostat.h"
extern Thermostat thermo;

// ============================================================================
//  HELPER GRAFICI (ICONE)
// ============================================================================
...
// ============================================================================
//  LOGICA PULSANTE MANUALE (DEMO)
// ============================================================================

// Funzione helper per aggiornare lo stile del bottone
void update_manual_toggle_btn() {
    if(!btn_manual_toggle || !lbl_manual_toggle) return;

    if (thermo.isHeatingState()) {
        // Caldaia ACCESA (Fa Caldo) -> Pulsante per spegnere
        ...
        // Caldaia SPENTA (Fa Freddo) -> Pulsante per accendere
...
static void manual_toggle_cb(lv_event_t * e) {
    // Toggle stato logico (per ora demo, ma comanda la classe Thermostat)
    if (thermo.isHeatingState()) {
        thermo.stopHeating();
    } else {
        thermo.startHeating();
...
void build_scr_main() {
    ...
    // NUOVO BOTTONE MANUALE (Posizionato a destra della data)
    btn_manual_toggle = lv_button_create(col_left);
    lv_obj_set_size(btn_manual_toggle, 220, 60);
    lv_obj_align(btn_manual_toggle, LV_ALIGN_TOP_RIGHT, -20, 50); // Allineato a destra nella colonna sinistra
    lv_obj_add_event_cb(btn_manual_toggle, manual_toggle_cb, LV_EVENT_CLICKED, NULL);
    // Stile iniziale (Freddo -> Accendi)
    ...
    // --- Meteo (Spostato un po' più giù per fare spazio al bottone) ---
    ...
    lv_obj_align(cont_current, LV_ALIGN_TOP_LEFT, 10, 130); // Abbassato da 110 a 130
    ...
    for(int i=0; i<5; i++) create_forecast_box(bot_section, i);
...
void update_ui() {
    ...
    // Aggiorna stato bottone manuale (solo se in home)
    if (act == scr_main) {
        update_manual_toggle_btn();
    }
